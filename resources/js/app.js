document.addEventListener('DOMContentLoaded', () => {
  ...
  // array path seperti yang diminta
  ...
  // tambahkan property url ke tiap product berdasarkan urutan productPaths
  ...
  // --- render produk (dapat dipanggil ulang) ---
  function renderProducts(list) {
    ...
    list.forEach(product => {
      ...
      card.dataset.url = product.url; // simpan url di dataset
      ...
      // klik card navigasi ke product.url
      card.addEventListener('click', (e) => {
        // jika klik tombol add-to-cart, jangan navigasi
        if (e.target.closest('.add-to-cart')) return;

        const url = card.dataset.url || '/';
        // jika user tekan Ctrl/Cmd klik, buka di tab baru
  ...
  // helper untuk safety output di innerHTML minimal
  ...
  if (input) {
    input.addEventListener('input', () => {
      const q = input.value;
      // jika kamu pakai suggestions, tetap munculkan suggestions (kode lama)
        ...
        if (!val) { sugBox.classList.add('hidden'); sugList.innerHTML = ''; }
          ...
          if (filteredSug.length === 0) { sugBox.classList.add('hidden'); sugList.innerHTML = ''; }
            ...
            sugList.querySelectorAll('li').forEach(li => li.addEventListener('click', () => {
              ...
              filterAndRender(input.value); // filter when suggestion chosen
            }));
          }
        }
      }
    ...
    // Enter key juga melakukan pencarian (prevent form submit bila ada)
    input.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        e.preventDefault();
        filterAndRender(input.value);
        // tutup suggestions kalau ada
        if (sugBox) sugBox.classList.add('hidden');
      }
    });
  }

  // search button (jika ada)
  if (searchBtn) {
  ...
  // tutup suggestion saat klik luar
  if (sugBox) {
